ALTER TABLE public.agent_meetings DROP CONSTRAINT IF EXISTS agent_meetings_max_specialists_check;
ALTER TABLE public.agent_meetings ADD CONSTRAINT agent_meetings_max_specialists_check CHECK (max_specialists >= 2 AND max_specialists <= 5);
ALTER TABLE public.agent_meetings DROP CONSTRAINT IF EXISTS agent_meetings_selected_roles_check;
ALTER TABLE public.agent_meetings ADD CONSTRAINT agent_meetings_selected_roles_check CHECK (cardinality(selected_roles) <= 5);
...
BEGIN
  ...
  IF p_run_kind = 'autonomous' THEN
    IF v_projected > p_soft_cap_sek THEN
      RETURN jsonb_build_object('ok', false, 'code', 'soft_paused', 'state', 'soft_paused', 'reason', 'Månadens mjuka kostnadstak är nått. Autonoma körningar pausas.');
    END IF;
    IF v_runs_day_role >= p_max_autonomous_runs_day THEN
      RETURN jsonb_build_object('ok', false, 'code', 'daily_run_cap', 'state', 'run_capped', 'reason', 'Dygnets tak för autonoma körningar är nått för den här agenten.');
    END IF;
    IF v_runs_month >= p_max_autonomous_runs_month THEN
      RETURN jsonb_build_object('ok', false, 'code', 'monthly_run_cap', 'state', 'run_capped', 'reason', 'Månadens tak för autonoma körningar är nått.');
    END IF;
  END IF;
