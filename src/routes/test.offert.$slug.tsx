...
export const Route = createFileRoute("/test/offert/$slug")({
  loader: async ({ params }) => {
  ...
  head: ({ loaderData }) => {
    const title = `Testversion – ${loaderData?.landing.name ?? "offertförfrågan"}`;
    const description = "Intern testversion av offertformuläret.";
    return {
      meta: [
        { title },
...
function Page() {
  ...
  return <DynamicLanding landing={landing} test />;
}
