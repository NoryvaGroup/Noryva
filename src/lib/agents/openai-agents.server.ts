...
export async function runHarnessSession(
  ...
  try {
    ...
    // Ingen session hann skapas: inget provider_run_id, inga tokens, retrybar.
    ...
    return { ...blocked(reason, agentId), runStatus: "failed", retryable: true, phase: "create" };
  }
  ...
  await deleteCompletedSession(doFetch, sessionId, headers, conflictBackoffMs);
